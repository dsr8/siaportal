...
function interview_ADR_completed_team_mail($iggd,$name,$date_int_req_rec){
  ...
  $From="Sia Immigration";
         ...
         // $headers.="From: \"".$From."\"<".$emaill."> \r\n";
          ...
          $message .= 'Request Completed Date : ' .$date_int_req_rec.'<br>';
...
//function interview_ADR_completed_client_mail_sms($name,$tname,$tmobile,$cmobile,$email){}
...
//@mail($email.',ds@siaimmigration.com,no-reply@siaimmigration.com',$subject,$message,$headers);
helper('smtp_helper');
  $email1 = \Config\Services::email();
    $config = get_smtp_settings();
    $email1->initialize($config);
