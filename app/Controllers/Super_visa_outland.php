...
class Super_visa_outland extends BaseController
{


public function edit_super_visa_outland($category,$id,$sid){
...
interview_ADR_requested_team_mail($iggd,$name,$date_int_req_com);
...
$data['cpml'] = $cpm->where('id',$id)
		...
		return view('admin/super_visa_outland/edit_super_visa_outland',$data);
}
}
///-------------------
...
public function edit_super_visa_outland_new($category,$id,$sid){
...
$data['cpml'] = $cpm->where('id',$id)
		...
		return view('admin/super_visa_outland/edit_super_visa_outland_new',$data);
}
}
///-------------------
//////////new end
...
public function full_super_visa_outland($category,$id,$sid){
    ...
    $Team = new Team_model();
    $data['team'] = $Team->where('id',$assign_to)->findAll();

    $vmm = new Voice_msg_model();
    $data['voice_msg'] = $vmm->where('client_application_id',$id)->findAll();

    $Prospect = new Prospect_model();
    $data['profile'] = $Prospect->where('id',$sid)->findAll();

    $cdm = new Client_document_model();
    $data['doc']  = $cdm->where('application_id',$id)->where('upload_by','Client')->findAll();
    $data['doc1'] = $cdm->where('application_id',$id)->where('upload_by','Sia')->findAll();
    ...
    $data['cpml'] = $cpm->where('id',$id)->findAll();

    return view('admin/super_visa_outland/full_view_super_visa_outland',$data);
}
