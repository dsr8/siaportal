...
public function move_to_prospect($id)
{
    ...
    $insert = $Prospect->insert([
        'heading'          => $record['heading'],
         ...
         'update_on'        => $record['update_on'],
        // 'voice_added'      => $record['voice_added'],
...
if($entries=='all'){
	...
	// Fetch ALL entries to build duplicate ID map (name+email OR name+dob)
	$db = \Config\Database::connect();
	$allForDup = $db->query("SELECT id, heading, email, user_dob FROM tbl_client_prospect WHERE entery_status = 'client'")->getResultArray();
	$nameEmailMap = [];
	$nameDobMap   = [];
	foreach ($allForDup as $r) {
		$name = strtolower(trim($r['heading']));
		if (!empty(trim($r['email']))) {
			$nameEmailMap[$name . '|' . strtolower(trim($r['email']))][] = $r['id'];
		}
		if (!empty(trim($r['user_dob'])) && $r['user_dob'] !== '0000-00-00') {
			$nameDobMap[$name . '|' . $r['user_dob']][] = $r['id'];
		}
	}
	// $dupIds[$id] = sorted array of all sibling duplicate IDs
	$dupIds = [];
	foreach (array_merge(array_values($nameEmailMap), array_values($nameDobMap)) as $ids) {
		if (count($ids) >= 2) {
			sort($ids);
			foreach ($ids as $id) {
				if (!isset($dupIds[$id])) $dupIds[$id] = [];
				$dupIds[$id] = array_unique(array_merge($dupIds[$id], array_diff($ids, [$id])));
			}
		}
	}
...
public function adr_mail(){
    ...
    // Fetch records where adr_start_date is today or within 3 days of adr_end_date
    $data['dob'] = $Adr->where('adr_start_date', $current_date)
...
$Prospect = new Prospect_model(); 
	...
	//->where('appo_book', 'Appointment booked')
			'prospect_all'=>$Prospect->select('id,news_image1,voice_added,from_web,heading,email,ccode,mail_send,mail_send_on,sms_send,sms_send_on,number,insert_on,pstatus,ppstatus,agent_name,typee,team_member,number,agent_status,admin_status')
...
$data = [
			//'prospect' => $paginateData,
	//->where('appo_book', 'Appointment booked')
			'prospect'=>$Prospect->select('id,news_image1,voice_added,from_web,heading,email,ccode,mail_send,mail_send_on,sms_send,sms_send_on,number,insert_on,pstatus,ppstatus,agent_name,typee,team_member,number,agent_status,admin_status')
			->where('entery_status', 'prospect')
		...
		->orderBy('id', 'desc')->findAll();
			...
			$voice=$data['daily_mail_send']['0']['news_image1'];
				...
				$From="Immigration Enquiry-".$name." ";
					...
					$subject  = "Name ".$name."-Id".$id."-Immigration Enquiry:- Processed/Delete";
		...
		->orderBy('id', 'desc')->findAll();
			 ...
			 $voice=$data['daily_mail_send']['0']['news_image1'];
				...
				$From="Immigration Enquiry records-".$name." ";
					 $emaill="apps@siaimmigration.com";
					$subject  = "Name ".$name."-Id".$id."-New Admission:- Dropped/Delete";
					//$subject  = "Lended  Application Fee";						
					$headers  = "MIME-Version: 1.0\r\n";
 					$headers .= "Content-type: text/html; charset=iso-8859-1\r\n"; 					
 				//	$headers.="From: \"".$From."\"<".$emaill."> \r\n";
		...
		->orderBy('id', 'desc')->findAll();
			 ...
			 $voice=$data['daily_mail_send']['0']['news_image1'];
				...
				$From="Immigration Enquiry records-".$name." ";
					...
					$subject  = "Name ".$name."-Id".$id."-New Admission:- Appointment Scheduled/ Delete";
