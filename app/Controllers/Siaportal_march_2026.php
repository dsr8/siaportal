...
class Siaportal extends BaseController
{
    ...
    {
        // Redirect to a specific page like assets page
        return redirect()->to('https://siaimmigration.com/');
    }
...
// Set email fields
$emailService->setFrom('no-reply@siaimmigration.com', $from);
...
if($entries=='all'){
	...
	// Fetch ALL entries (id, heading, user_dob only) to build duplicate map
	$db = \Config\Database::connect();
	$allForDup = $db->query("SELECT id, heading, user_dob FROM tbl_client_prospect WHERE entery_status = 'client'")->getResultArray();
	$dupMap = [];
	foreach ($allForDup as $r) {
		$key = $r['heading'] . '|' . $r['user_dob'];
		$dupMap[$key][] = $r['id'];
	}
	foreach ($dupMap as $key => $ids) {
		if (count($ids) < 2) unset($dupMap[$key]);
	}
...
public function adr_mail(){
    ...
    foreach($data['dob'] as $db) {
        ...
        // On the last date (adr_end_date), send two emails 4 hours apart
        if ($db['adr_end_date'] == $current_date) {
            // Send first mail now
            $email1->send();

            // Schedule another email to be sent 4 hours later
...
$message1 = preg_replace('/\\\\/', '', $message);
						$to = array('no-reply@siaimmigration.com');
				//$cc = ['office@siaimmigration.com','admin@siaimmigration.com','info@siaimmigration.com','kam@siaimmigration.com','mkj@siaimmigration.com','ds@siaimmigration.com','Reach@siaimmigration.com','support@siaimmigration.com'];
