...
if ($this->request->getMethod()=='post'){
 ...
 if( $this->request->getFileMultiple('resume'))
        {
$iq=0;
        	

        	 foreach($this->request->getFileMultiple('resume') as $img)
             { 
            ...
            if ($img->isValid() && ! $img->hasMoved())
            {
                ...
                //echo $newName;
               // exit();
	$dcn=$this->request->getPost('doc_name');
...
$link='https://canada.siaimmigration.com/assets/resume/'.$newName;
	...
	'siaportal_id'=>$this->request->getPost('siaprotal_id'),
...
$From="Sia Immigration";
            ...
            'X-Mailer: PHP/' . phpversion();
					...
					$message .= 'Document name : ' .$dcn[$iq].'<br>';
